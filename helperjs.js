...
window.addEventListener("load",function(event) {
  document.querySelectorAll('[repeater*=true],[money-format*=true],[numeric*=true],[character-count*=true],[helper-if],[date*=true],[str-replace*="["],[striptags*=true],[htmlentities*=true],[substr*=true],[function],[md5*=true],[load-file],[include],[uppercase*=true],[iframe-open*=true],[img2canvas*=true],[google-map*=true],[timer*=true]').forEach(function(x){
    ...
    */
    ...
    // Example 2 Result : <a helper-if=" !href ? javascript:void(0);" href="http://helperjs.com">Link 2</a>
    // Example 3 : <a helper-if=" href ? javascript:void(0);" href="http://helperjs.com">Link 3</a>
    // Example 3 Result : <a helper-if=" !href ? javascript:void(0);" href="javascript:void(0);">Link 3</a>
    if(x.getAttribute('helper-if')){
      try {
        ...
        if(_attrName.indexOf('!') > -1){
          if(!x.getAttribute(_attrName.replace('!','').trim())){
            x.setAttribute(_attrName.replace('!','').trim(), helper_if[1]);
          }
        }
        else {
    ...
    //htmlentities
    if(x.getAttribute('htmlentities') == 'true'){
      if(elementArray.indexOf(x.nodeName.toLowerCase()) > -1 && x.innerHTML){
        x.innerHTML =x.innerHTML.trim().str_replace(['<','>'],['&lt','&gt']);
      }
      else if((x.nodeName.toLowerCase() == 'input' || x.type.toLowerCase() == 'text') && x.getAttribute('value')){
        x.setAttribute('value',x.getAttribute('value').trim().str_replace(['<','>'],['&lt','&gt']))
      }
      else if(x.nodeName.toLowerCase() == 'textarea' && x.textContent){
        x.textContent = x.textContent.trim().str_replace(['<','>'],['&lt','&gt']);
      }
    }
    //substr
    if(x.getAttribute('substr') == 'true'){
    ...
    //load file
    if(x.getAttribute('load-file') == 'true'){
    ...
    //include file
    if(x.getAttribute('include') == 'true'){
    ...
    if(x.getAttribute('uppercase') == 'true'){
    ...
    //image to canvas
    if(x.getAttribute('img2canvas') == 'true'){
      var id = helperjs.MD5(Math.random().toString(36).substring(7));
    ...
    //date
    if(x.getAttribute('date') == 'true'){
      try {
        var today = helperjs.date({
          seperator : x.getAttribute('date-seperator') ? x.getAttribute('date-seperator') : '',
          format : x.getAttribute('date-format') ? x.getAttribute('date-format') : ''
        });
        if(elementArray.indexOf(x.nodeName.toLowerCase()) > -1){
            x.innerHTML = today;
        }
        else if(x.nodeName.toLowerCase() == 'input' || x.type.toLowerCase() == 'text'){
          x.setAttribute('value',today);
        }
        else if(x.nodeName.toLowerCase() == 'textarea' ){
          x.textContent = today;
        }
...
var helperjs = {
  ...
  timer: function (data) {
    ...
    setInterval(function () {
      ...
      if (--timer < 0) {
        timer = 0;
        i == 1 && data.callback && data.callback();
        i = 0;
      }
    }, 1000);
  },
  ...
  //date formats
  //default : DDMMYYYY
  //YYYYMMDD || YYYYDDMM || MMDDYYYY
  date : function(data){
    var seperator = data.seperator ? data.seperator : '.';
    var date = new Date();
    ...
    if(data.format.toLowerCase() == 'yyyymmdd'){
      var today = year+seperator+month+seperator+day;
    }
    else if(data.format.toLowerCase() == 'yyyyddmm'){
      var today = year+seperator+day+seperator+month;
    }
    else if(data.format.toLowerCase() == 'mmddyyyyy'){
      var today = month+seperator+day+seperator+year;
    }
    else {
      var today = day+seperator+month+seperator+year;
    }
    return today;
  },
  //md5 encryption
